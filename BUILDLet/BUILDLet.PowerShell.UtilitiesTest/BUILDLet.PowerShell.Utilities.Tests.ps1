...
$load_test_target = @{
#
    'Invoke-Process1' = $false  # Take a few minutes
    'Invoke-Process2' = $false  # Take a few minutes
#>
}
...
if ($test_target['Get-FileVersionInfo'] -or `
    ...
    # Copy test DLL file
    if (-not (Test-Path -Path $win32_test_file)) { Copy-Item -Path "..\..\..\TestData\$win32_test_file" } 
...
if ($test_target[$target]) {
    ...
    Describe "$target Cmdlet" {
	    ...
	    Context "File" {
            ...
            $test_files = @{
                'Relative Path' = '..\Release\BUILDLet.PowerShell.Utilities\BUILDLet.PowerShell.Utilities.dll'
	    ...
	    Context "Folder" {
            ...
            $test_folders = @{
                'Relative Path' = '..\Release\BUILDLet.PowerShell.Utilities'
...
if ($test_target[$target]) {
    ...
    Describe "$target Cmdlet" {
        ...
        # Test Files
        $html_files = @(
            '..\..\..\TestData\W3C_example.html'
            '..\..\..\TestData\Ç∆ÇŸÇŸ_ÉTÉìÉvÉã.html'
            '..\..\..\TestData\HTML 4_01 Specification.htm'
            '..\..\..\TestData\comment.html'
        )
...
if ($test_target[$target]) {
    ...
    Describe "$target Cmdlet" {
        ...
        if (Test-Path -Path $ini_files[1]) { Remove-Item $ini_files[1] -Force }
        Copy-Item -Path $ini_files[0] -Destination $ini_files[1]
...
if ($load_test_target[$target + '1']) {
    ...
    Describe "$target Cmdlet (Load Test 1)" {
        ...
        # Test Files
        $exe_files = @(
            '..\TestData\stdout300.exe'
            '..\TestData\stderr300.exe'
            '..\TestData\randum300.exe'
        )
